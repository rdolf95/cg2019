...
def render():
    global gCamAng, gCamHeight, diffuseLightColor, specularLightColor, ambientObjectColor, diffuseObjectColor
    ...
    glMaterialfv(GL_FRONT, GL_AMBIENT, ambientObjectColor)
    glMaterialfv(GL_FRONT, GL_DIFFUSE, diffuseObjectColor)
    glMaterialfv(GL_FRONT, GL_SPECULAR, specularObjectColor)
...
def key_callback(window, key, scancode, action, mods):
    ...
    if action==glfw.PRESS or action==glfw.REPEAT:
        if key==glfw.KEY_1:
            ...
            ambientObjectColor = (1., 0., 0., 1.)
            diffuseObjectColor = (1., 0., 0., 1.)
        elif key == glfw.KEY_X:
            ambientObjectColor = (0., 1., 0., 1.)
            diffuseObjectColor = (0., 1., 0., 1.)
        elif key == glfw.KEY_C:
            ambientObjectColor = (0., 0., 1., 1.)
            diffuseObjectColor = (0., 0., 1., 1.)
        elif key == glfw.KEY_V:
            ambientObjectColor = (1., 1., 1., 1.)
            diffuseObjectColor = (1., 1., 1., 1.)
...
def main():
    ...
    if not glfw.init():
        return
    window = glfw.create_window(480,480,'2015004493-8-1', None,None)
    if not window:
        glfw.terminate()
        return
