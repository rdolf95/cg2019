...
def log(R):
    ...
    v1 = (R[2, 1] - R[1, 2])/(2*np.sin(ang))
    v2 = (R[0, 2] - R[2, 0])/(2*np.sin(ang))
    v3 = (R[1, 0] - R[0, 1])/(2*np.sin(ang))
...
def interpolateZYXEuler(euler1, euler2, t):
    ...
    euler = (euler_v[0], euler_v[1], euler_v[2])
...
def main():
    ...
    if not glfw.init():
        return
    window = glfw.create_window(480,480,'2015004493-10-1', None,None)
    if not window:
        glfw.terminate()
        return
